# Домашнее задание к занятию "`Резервное копирование баз данных`" - `Осаковская Анна`
...
mysqldump -u root -p --all-databases --flush-logs --master-data=2 \
  --single-transaction --routines --triggers --events > /backup/full_backup_$(date +%Y%m%d).sql
```
...
```
*Восстановление из полного бекапа + бинарные логи*
```
mysql -u root -p < /backup/full_backup_20231015.sql

mysql -u root -p < /backup/incremental_backup_20231015_1400.sql
mysql -u root -p < /backup/incremental_backup_20231015_1500.sql
```
***3.2 Преимущества использования реплики перед обычным резервным копированием**
1. При обычном бекапе сосдается нагрузка на мастер — блокировки, потребление ресурсов, риск таймаутов. Реплика позволяет снимать бекапы с копии базы, где мастер продолжает работать без каких-либо замедлений для пользователей.

2. При отказе мастера восстановление из бекапа занимает часы: развёртывание нового сервера, копирование данных, применение логов. Реплика даёт возможность переключить трафик за секунды. 

3. Реплику можно разместить в другом дата-центре или регионе. Если произойдёт чрезвыяайная систуация или отключение ЦОДа — бизнес продолжит работу. 
