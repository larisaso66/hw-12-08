...
Полное резервное копирование (еженедельно) + инкрементное/дифференциальное копирование (каждый час). Позволяет восстановить данные на момент максимально близкий к сбою. Для восстановления потребуется последний полный бекап + все инкрементные бекапы до нужного часа.
